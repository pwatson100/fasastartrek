...
const NAME = 'fasastartrek';
const TITLE = 'FASA Star Trek';
...
export class COMMON {
	...
	static applySettings(settingsData, moduleKey = COMMON.DATA.name) {
		Object.entries(settingsData).forEach(([key, data]) => {
			game.settings.register(moduleKey, key, {
				name: COMMON.localize(`fasastartrek.${key}.name`),
				hint: COMMON.localize(`fasastartrek.${key}.hint`),
				...data,
			});
		});
	}
	...
	static addAbstract(cls, key) {
		if (!globalThis.game.fasastartrek.abstract) globalThis.game.fasastartrek.abstract = {};
		globalThis.game.fasastartrek.abstract[key] = cls;
	}
}
